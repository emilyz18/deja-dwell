...
const tenantPrefQueries = {
  editTenantPref: async function (preferenceID, data) {
    try {
      return await TenantPreference.findOneAndUpdate(
      ...
      throw new Error(
        'Failed updating tenant preference to mongoDB' + error.message
      )
    }
  },
  getOneTenantPref: async function (preferenceID) {
    try {
      ...
      throw new Error(
        'Failed to find tenant prefernece using ID, ' + err.message
      )
    }
  },
  ...
  creatTenantPref: async function (data) {
    try {
      ...
      throw new Error(
        'Failed create tenant preference when sign up' + error.message
      )
    }
  },
}
