...
router.get('/landlord/:landlordId', async (req, res) => {
  ...
  try {
    ...
    const activedMatches = matches.filter(match => (match.MatchStatus !== 'Rejected' && match.MatchStatus !== 'Disliked' && match.LandlordID === landlordId));
    ...
    const combinedResults = activedMatches.map(match => {
      ...
      const tenantPreference = tenantProfile ? tenantPrefs.find(pref => pref.TenantPreferenceID === tenantProfile.TenantPreferenceID) : null;
      ...
      return {
        ...
        lengthOfLease: tenantPreference ? tenantPreference.Duration : null,
